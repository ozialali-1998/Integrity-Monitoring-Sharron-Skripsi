...
def _install_sqlalchemy_stub() -> None:
    ...
    class _DriverSqlResult:
        def fetchall(self) -> list[Any]:
            return []

    class _EngineConnection:
        def __enter__(self) -> "_EngineConnection":
            return self

        def __exit__(self, exc_type, exc_value, traceback) -> None:
            pass

        def exec_driver_sql(self, statement: str) -> _DriverSqlResult:
            return _DriverSqlResult()

    class _Engine:
        def begin(self) -> _EngineConnection:
            return _EngineConnection()
...
def _install_psutil_stub() -> None:
    ...
    class _MemoryInfo:
        def __init__(self, rss: int = 123456):
            self.rss = rss
