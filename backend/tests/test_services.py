...
def test_run_benchmark_persists_result(tmp_path: Path) -> None:
    ...
    assert results[0].verification_duration_ms is not None
    assert results[0].accuracy_percent == 100.0
    assert results[0].verification_matches == 1
    assert results[0].verification_mismatches == 0
    assert results[0].average_cpu_percent is not None
    assert results[0].max_cpu_percent is not None
    assert results[0].memory_peak_bytes is not None
    assert db.commits == 1
